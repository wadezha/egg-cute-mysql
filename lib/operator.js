...
class Operator {
  ...
  async page(sql, values, countSql = '') {
    ...
    values = values || {};
    if (!values.size || !regex.test(values.size)) {
      values.size = this.size;
    }
    if (!values.pageNum || !regex.test(values.pageNum)) {
      values.pageNum = 1;
    }
    values.size = Number.parseInt(`${values.size}`, 10);
    values.pageNum = Number.parseInt(`${values.pageNum}`, 10);
    const offset = (values.pageNum - 1) * values.size;

    const sqlLimit = this.limit(values.size, offset);
