...
class Operator {
  constructor(config) {
    ...
    this.selectKey = config.selectKey || 'Camelcase';
    delete config.pageSize;
    delete config.selectKey;
  }
  ...
  async info(sql, values) {
    ...
    const rows = await this.query(sqlQuery);
    const result = (rows && rows.length > 0) ? rows[0] : undefined;
    return this.selectKey === 'Camelcase' ? this[camelCaseObjectKeys](result) : result;
  }

  async list(sql, values) {
    const sqlQuery = this.format(sql, values);
    const rows = await this.query(sqlQuery);
    return this.selectKey === 'Camelcase' ? rows.map(m => this[camelCaseObjectKeys](m)) : rows;
  };
  ...
  async insert(sql, values) {
    ...
    // console.log((result && result.hasOwnProperty('insertId')) ? result.insertId : 0); // ?
    return result;
  }
  ...
  async update(sql, values) {
    const sqlQuery = this.format(sql, values);
    const result = await this.query(sqlQuery);
    // console.log((result && result.hasOwnProperty('changedRows')) ? result.changedRows : 0); // ?
    return result;
  }
  ...
  async delete(sql, values) {
    const sqlQuery = this.format(sql, values);
    const result = await this.query(sqlQuery);
    // console.log((result && result.hasOwnProperty('affectedRows')) ? result.affectedRows : 0); // ?
    return result;
  }
  ...
  async page(sql, values, countSql = '') {
    ...
    values = values || {};
    if (!values.pageSize || !regex.test(values.pageSize)) {
      values.pageSize = this.pageSize;
    }
    if (!values.pageNum || !regex.test(values.pageNum)) {
      values.pageNum = 1;
    }
    values.pageSize = Number.parseInt(`${values.pageSize}`, 10);
    values.pageNum = Number.parseInt(`${values.pageNum}`, 10);
    const offset = (values.pageNum - 1) * values.pageSize;

    const sqlLimit = this.limit(values.pageSize, offset);
